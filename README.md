...
│   └── spirit.db             # Memória local SQLite (gerado automaticamente)
...
Crie o arquivo `backend/.env` com sua chave pessoal:
...
> **Segurança:** `NVIDIA_API_KEY` é uma chave pessoal e nunca deve ser incluída em commits, issues, capturas de tela ou código do frontend. Mantenha `.env` no `.gitignore` e use `.env.example` para documentar somente os nomes das variáveis.
...
Para servir o frontend localmente, em outro terminal:
...
Abra `http://127.0.0.1:5500` no navegador. Caso use outra porta, inclua a origem correspondente em `CORS_ORIGINS`.

## API

| Método | Endpoint | Descrição |
|---|---|---|
| `POST` | `/chat` | Envia uma mensagem para a Spirit. A resposta é transmitida como NDJSON, com eventos `reasoning` e `content`; o identificador da conversa vem no cabeçalho `X-Conversation-ID`. |
...
| `GET` | `/conversations` | Lista conversas salvas na memória SQLite. Aceita `limit` opcional, de 1 a 100. |
| `GET` | `/conversations/{conversation_id}/messages` | Retorna as mensagens persistidas de uma conversa específica. |
...
```powershell
Invoke-WebRequest -Method Post http://127.0.0.1:8000/chat `
  -ContentType 'application/json' `
  -Body '{"message":"Olá, Spirit!"}'
```
...
O banco contém o histórico local da aplicação. Faça backup se desejar preservá-lo e remova-o somente se quiser apagar as conversas salvas.
